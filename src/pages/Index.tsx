...
import { Clock, FileText, CheckCircle, XCircle, Upload, Settings, RotateCcw, ArrowLeft, ArrowRight } from 'lucide-react';
...
const TestApplication = () => {
  const [currentScreen, setCurrentScreen] = useState<'welcome' | 'setup' | 'test' | 'results'>('welcome');
  ...
  const goToQuestion = (index: number) => {
    ...
    const existingAnswer = userAnswers.find(a => a.questionId === testQuestions[index].id);
                ...
                <div className="space-y-3">
                  ...
                  {testData && (
                    <div className="p-3 bg-success-100 rounded-lg border border-success-500">
                      <p className="font-medium text-success-600">{testData.title}</p>
                      <p className="text-sm text-success-600">{testData.questions.length} questions available</p>
                    </div>
                  )}
                </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setTestConfig({
                            ...testConfig,
                            numberOfQuestions: Math.min(parseInt(e.target.value) || 1, testData.questions.length)
                          })}
                          className="mt-1"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setTestConfig({
                            ...testConfig,
                            startFrom: Math.min(parseInt(e.target.value) || 1, testData.questions.length)
                          })}
                          className="mt-1"
                        />
                      </div>
                ...
                <CardContent>
                  <ScrollArea className="h-64">
                    <div className="grid grid-cols-5 gap-2">
                      {testQuestions.map((_, index) => {
                        const isAnswered = userAnswers.some(a => a.questionId === testQuestions[index].id);
                  ...
                  <div className="mt-4 pt-4 border-t border-warm-200">
                    <div className="text-sm text-warm-600 space-y-1">
                      <p>Answered: {userAnswers.length} / {testQuestions.length}</p>
                      <Button
                        onClick={finishTest}
                        variant="outline"
                        size="sm"
  ...
  if (currentScreen === 'results') {
    ...
    const testDuration = endTime && startTime ? Math.floor((endTime.getTime() - startTime.getTime()) / 1000) : 0;
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-3">
                  {currentQuestion.options.map((option, index) => {
                    ...
                    } else if (userAnswer && index === userAnswer.selectedOption && !userAnswer.isCorrect) {
                      ...
                      <div key={index} className={className}>
                        <div className="flex items-center gap-3">
                          <div className="flex items-center gap-2">
                            ...
                            {userAnswer && index === userAnswer.selectedOption && !userAnswer.isCorrect && (
                ...
                {currentQuestion.explanation && (
                  <div className="p-4 bg-sage-50 rounded-lg border border-sage-200">
                    <h4 className="font-medium text-sage-700 mb-2">Explanation</h4>
                    <p className="text-sage-600">{currentQuestion.explanation}</p>
                  </div>
                )}
            ...
            <Card className="test-card text-center">
              <CardContent className="pt-6">
                <div className={`text-2xl font-bold mb-1 ${
                  results.percentage >= 70 ? 'text-success-500' : 'text-error-500'
                }`}>
                  {results.grade}
                </div>
